...
function searchDeck() {
   ...
   location.href = `${address}/make2/?hpUp=${hpUp}&fit13t=${fit13t}&list=${go}&bond=${b}&ess=${essential}`;
}
...
// 필수 캐릭터 창에 선택된 캐릭터 이미지 띄우기
function updateEssential() {
   const div = document.getElementById("essentialCh");
   if (essential.length == 0) div.innerHTML = "";
   else {
      let innerArray = [];
      for(let chId of essential) {
         let champ = getCharacter(chId);
         let id = champ.id, name = champ.name, element = champ.element, role = champ.role;
         innerArray.push(`
            <div class="character" onclick="clickedEss(this, ${id})" style="margin:0.2rem;">
               <div style="position:relative; padding:0.2rem;">
                  <img src="${address}/images/characters/cs${id}_0_0.webp" class="img z-1" alt="">
...
function clickedCh(id) {
   ...
   if (index === -1) {
      ...
      let essidx = essential.indexOf(id);
      if (essidx != -1) {
         essential.splice(essidx, 1);
         updateEssential();
      }
...
function clickedEss(div, id) {
   let index = essential.indexOf(id);
   if (index != -1) essential.splice(index, 1);
   updateEssential();
}
...
function clickedSel(div, id) {
   ...
   if (essentialOn) {
      if (essential.includes(id)) return;
      essential.push(selected[index]);
      updateEssential();
      return;
   }
...
// 필수캐릭터 설정
let essentialOn = false;
function setEssential() {
   essentialOn = !essentialOn;
   const scc = document.getElementById("selectedChContainer");
   const btn = document.getElementById("essentialBtn");
   if (essentialOn) {
      scc.style.borderColor = "white";
      btn.innerText = "OK";
      btn.style.backgroundColor = "limegreen";
   } else {
      scc.style.borderColor = "transparent";
      btn.innerText = "SET";
