...
function setPossible() {
   ...
   if (compList.every(item => haveList.includes(item))) {
      ...
      if (dataIdx == maxDataCnt) makeBlockByModNSort();
...
/* 백트래킹 함수 -----------------------------------------------------------*/
let progress = 0;
function backtrack0(possibleDeck) {
   let half = Math.round(possibleDeck.length / 2);
   backtrackOneCycle(half-1, false);
   backtrackOneCycle(half, true);
}

function backtrackOneCycle(i, isUp) {
   if (i < 0 || i >= possibleDeck.length) return;
   ...
   if (progress >= possibleDeck.length) makeBlockNDeck();
      ...
      if (isUp) setTimeout(() => backtrackOneCycle(i+1), 16);
      else setTimeout(() => backtrackOneCycle(i-1), 16);
   }
}
