   ...
   for(const id of list) {
      ...
      stringArr.push(`
         <div style="display:flex; flex-direction:column; align-items:center">
            ...
            <button class="setting-btn" onclick="open_p(${idx})">${t("설정")}</button>
...
function makePotentialUI() {
   ...
   radios.forEach(function(option) {
      option.addEventListener("change", function() {
         ...
         document.getElementById("di-txt").innerText = ability_save[curIdx].discipline;
      });
   });
...
function click_potential(y, x) {
   ...
   document.getElementById("po-txt").innerText = y+1;
}
